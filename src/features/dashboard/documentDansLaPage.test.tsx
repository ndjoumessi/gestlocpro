...
/** Le fichier produit par un bouton, en octets. */
async function telecharger(nom: RegExp) {
  ...
  try {
    ...
    return textesPoses(fichier.bytes)
  } finally {
    capture.restore()
  }
}

describe('un document tient dans sa page', () => {
  it('ne pose aucun texte hors des marges', async () => {
    const textes = await telecharger(/^Télécharger$/)
    ...
    */
    ...
    for (const attendu of [NOM_LONG, IMMEUBLE_LONG, 'VIR-0000000000000000-REFERENCE-INTERMINABLE'])
      expect(
        recompose.includes(attendu),
        `le jeu d’essai n’atteint pas le document : « ${attendu} »`,
      ).toBe(true)
